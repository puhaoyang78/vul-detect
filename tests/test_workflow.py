...
class CandidatePolicyTests(unittest.TestCase):
    def test_recursive_expansion_follows_memory_relevance(self):
...
"""
        ...
        self.assertEqual({"helper", "relevant"}, {c.function.name for c in discovery.candidates})
...
class StandardSemanticTests(unittest.TestCase):
    def test_strcpy_wrapper_has_read_write(self):
        function = parse_functions(
            "copy.c", "void copy(char *d, const char *s) { strcpy(d, s); }\n"
        )[0]
        summaries = summaries_for_function(function)
        self.assertIn(
            {"kind": "WRITE", "buffer": "arg0", "length": "strlen(arg1) + 1"}, summaries
        )
        self.assertIn(
            {"kind": "READ", "buffer": "arg1", "length": "strlen(arg1) + 1"}, summaries
        )
...
class SliceTests(unittest.TestCase):
    def test_large_function_slice_keeps_reaching_definition(self):
        ...
        source = "void big(char *dst, int n) {\nint len = n + 1;\n" + filler + "\ncustom_copy(dst, len);\n}"
...
class WorkflowStateTests(unittest.TestCase):
    def test_upsert_preserves_unfinished_selected_samples(self):
        old = [
            {"sample_key": "S01", "value": "old-1"},
            {"sample_key": "S02", "value": "old-2"},
            {"sample_key": "S03", "value": "old-3"},
        ]
        new = [{"sample_key": "S01", "value": "new-1"}]
        result = _upsert_by_sample(old, new, {"S01", "S02"})
        by_key = {record["sample_key"]: record["value"] for record in result}
        self.assertEqual("new-1", by_key["S01"])
    ...
    def test_subset_refresh_preserves_unselected_normalization(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            args = SimpleNamespace(
                samples=str(samples_path), output=str(output_path), refresh=True,
                llm_backend="api", local_model="/unused/model.gguf",
                llama_server="/unused/llama-server", joern_dir="/unused",
                java_home="/unused", cpg_cache_dir=str(root / "cpg"),
            )
            with patch(
            ...
            self.assertEqual(
                {"S01", "S99"},
                {record["sample_key"] for record in read_jsonl(output_path)},
            )
